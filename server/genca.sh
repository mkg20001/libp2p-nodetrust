...
openssl req -x509 -newkey rsa:2048 -keyout wildkey.pem -nodes -out wildcert.pem -days 3650 -subj '/C=US/ST=Oregon/L=Portland/O=Libp2p Nodetrust DEV WILDCARD/OU=Org/CN=*.node.libp2p'
openssl x509 -in wildcert.pem -inform PEM -out wild.crt

# to install ca
# sudo cp {wild,ca}.crt /usr/share/ca-certificates/extra/nodetrust
